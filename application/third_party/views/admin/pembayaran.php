                            ...
                            <table class="table table-bordered table-hover table-striped">
                                <?php foreach ($sewa->result_array() as $row) { ?>
                                    ...
                                    <tr>
                                        <td>Status</td>
                                        <td>:</td>
                                        <td><div class="label label-<?= ($row['status'] == 'Lunas') ? 'success' : 'danger' ?>"><?= $row['status'] ?></div></td>
                                    </tr>
                                <?php } ?>
                            </table>
                        </div>
                ...
                <div class="box">
                    <?php if($this->session->userdata('level') == 'Administrator') { ?>
                        <div class="box-header">
                            ...
                            <a href="<?= base_url('admin/angsuran/cetakpembayaran/').$idSewa ?>" class="btn btn-warning" target="blank">
                                        ...
                                        <tr>
                                            ...
                                            <?php if($this->session->userdata('level') == 'Administrator') { ?>
                                                <td>
                                                    ...
                                                    <a href="<?= base_url('admin/angsuran/delete/').$ang->id.'/'.$idSewa ?>" class="btn btn-danger btn-xs tombol-yakin" data-isidata="Ingin menghapus data ini?">
...
<div class="modal fade" id="tambahData" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <form action="<?= base_url('admin/angsuran/insert/').$idSewa ?>" method="POST">
    ...
    <div class="modal fade" id="editData<?= $edit->id ?>" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form action="<?= base_url('admin/angsuran/update/').$idSewa.'/'.$edit->id ?>" method="POST">
                    <input type="hidden" name="<?= $this->security->get_csrf_token_name();?>" value="<?=$this->security->get_csrf_hash();?>" style="display: none">
                    <div class="modal-body">
                        <div class="form-group">
                            <label>Tanggal</label>
                        ...
                        <div class="form-group">
                            <label>Nominal</label>
                            <input type="number" name="nominal" class="form-control" placeholder="Nominal" value="<?= $edit->nominal ?>" required>
                        </div>
                        <div class="form-group">
                            <label>Keterangan</label>
