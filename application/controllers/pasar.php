...
class pasar extends CI_Controller {
	...
	function hitungKiosKosong($data, &$counts) {
    foreach ($data as $element) {
        // Cek jika elemen adalah kios
        if (isset($element['idKios']) && isset($element['status'])) {
            if ($element['status'] === 'kosong' && isset($element['jKios'])) {
                $jenis = $element['jKios'];
                // Tambah hitungan jika jenisnya terdaftar di $jenisKiosInfo
                if (array_key_exists($jenis, $counts)) {
                    $counts[$jenis]['kosong_count']++;
                }
            }
        } 
        // Jika elemen adalah array, panggil fungsi ini lagi untuk elemen tersebut
        elseif (is_array($element)) {
            hitungKiosKosong($element, $counts);
        }
    }
}
